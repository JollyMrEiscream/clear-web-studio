(function () {
  ...
  function syncStatusConnected() {
    setSyncStatus("Synced (" + new Date().toLocaleTimeString() + ")", "sync-status-ok");
  }
  ...
  async function pullFromGitHub() {
    ...
    if (!token) {
      setSyncStatus("Not connected", "sync-status-idle");
      return null;
    }
    setSyncStatus("Syncing…", "sync-status-pending");
  ...
  function scheduleGithubPush() {
    ...
    githubPushTimer = window.setTimeout(function () {
      pushToGitHub().catch(function (error) {
        setSyncStatus("Error: " + getErrorMessage(error), "sync-status-error");
  ...
  function renderSyncSettings() {
    ...
    if (!getGithubToken()) setSyncStatus("Not connected", "sync-status-idle");
  }
  ...
  function syncNow() {
    ...
    if (!getGithubToken()) {
      setSyncStatus("Not connected", "sync-status-idle");
      alert("Add a GitHub token in Cloud sync first.");
      return;
    }
    saveState({ sync: false });
    pushToGitHub()
      .then(function (ok) {
        if (ok) alert(storageMessage());
      })
  ...
  function fillSelect(id, values, options) {
    ...
    values.forEach(function (item) {
      ...
      if (item && typeof item === "object") {
        value = opts.valueKey ? item[opts.valueKey] : item.value != null ? item.value : item.name;
        label = opts.labelKey ? item[opts.labelKey] : item.label != null ? item.label : item.name != null ? item.name : value;
      } else {
        value = item;
        label = item;
      }
  ...
  function prospectFromPlace(place, category, area) {
    ...
    return {
      ...
      mapsUrl: mapsUrl,
      status: "New",
      opportunity: !website ? "no-website" : websiteLooksOutdated(website) ? "outdated" : "has-website",
      notes: !website
  ...
  function prospectMatchesFilter(prospect, filter) {
    if (filter === "no-website") return !prospect.website || prospect.opportunity === "no-website";
    if (filter === "outdated") {
      return (
        prospect.opportunity === "outdated" ||
        (!prospect.website || websiteLooksOutdated(prospect.website))
      );
    }
    if (filter === "any-opportunity") {
      return !prospect.website || websiteLooksOutdated(prospect.website) || prospect.opportunity !== "has-website";
    }
    return true;
  }

  function opportunityLabel(value) {
    const match = OPPORTUNITIES.find(function (item) {
  ...
  function renderProspectStats() {
    ...
    const noWebsite = state.prospects.filter(function (prospect) {
      return prospect.opportunity === "no-website" || !prospect.website;
    }).length;
    const outdated = state.prospects.filter(function (prospect) {
      return prospect.opportunity === "outdated";
    }).length;
    ...
    tbody.innerHTML = rows
      .map(function (prospect) {
        return (
          ...
          "</td>" +
          "<td>" +
          escapeHtml(opportunityLabel(prospect.opportunity)) +
          "</td>" +
          '<td class="actions">' +
  ...
  function formToProspect() {
    return {
      ...
      opportunity: getValue("prospect-opportunity-input") || "no-website",
  ...
  function openProspectModal(prospect) {
    ...
    setValue("prospect-opportunity-input", item.opportunity || "no-website");
